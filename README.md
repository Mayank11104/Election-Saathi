...
|---|---|---|
| Google Gemini API (gemini-1.5-flash) | Core AI engine — answers election questions in Hindi & English | src/api/gemini.ts |
| Google Analytics 4 | User behavior tracking — sessions, messages, language changes, chip clicks | src/utils/analytics.ts |
| Google Fonts API (Noto Sans Devanagari) | Multilingual typography — proper Hindi/Devanagari script rendering | index.html + MessageFormatter.tsx |
| Google Cloud Run | Serverless deployment and auto-scaling hosting | Backend deployment |
...
- **Rich Message Formatter** — AI responses are parsed and rendered with:
  - 🟠 Saffron numbered step badges for tutorials
  - 🟠 Clean bullet lists for documents
  - 🔗 Green portal cards for official ECI links
  - ⚠️ Amber warning boxes for important notices
  - 💡 Blue Pro Tip cards for helpful hints
  - ✅ Green Success lines for verified facts
  - 📝 Inline badges for specific forms (Form 6, EPIC, EVM, etc.)
...
- Fully containerized microservices architecture (Docker)
- **Frontend Container:** Multi-stage React build served by Nginx proxy
- **Backend Container:** FastAPI server with Uvicorn
- Seamless Nginx Reverse Proxy configured to handle CORS and route `/api/*` traffic to the backend seamlessly
- Successfully deployed and live on **Google Cloud Run** in `asia-south1`
- Environment variables and API keys securely injected at runtime
...
| Markdown Parsing | react-markdown + Custom Regex Renderer |
| Icons | Lucide React |
...
- `React.memo` on all major components for optimized re-renders
